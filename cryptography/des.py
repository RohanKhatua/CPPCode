def hex_to_bin(s):
    return ''.join(format(int(x, 16), '04b') for x in s)
...
def dec_to_bin(num, padding=4):
    return format(num, '0{}b'.format(padding))
...
def shift_left(k, nth_shifts):
    n = len(k)
    nth_shifts %= n  # handle shifts greater than length of k
...
def xor(a, b):
    ans = ''.join('0' if a[i] == b[i] else '1' for i in range(len(a)))
    return ans
